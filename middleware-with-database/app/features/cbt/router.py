...
async def get_cbt_schedules(
    ...
    try:
        ...
        logger.error(f"[cbt/schedules] Error uid={creds.get('uid')}: {e}")
        raise HTTPException(
...
async def verify_exam_token(
    ...
    try:
        ...
        logger.error(f"[cbt/verify-token] Error uid={creds.get('uid')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_exam_questions(
    ...
    try:
        repo = CbtRepository(db)
        service = CbtService(repo)
        data = await service.get_exam_questions(jadwal_id=jadwal_id)
        ...
        logger.error(f"[cbt/questions/{jadwal_id}] Error uid={creds.get('uid')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def submit_exam(
    ...
    try:
        repo = CbtRepository(db)
        service = CbtService(repo)
        data = await service.submit_exam(
        ...
        logger.error(f"[cbt/submit] Error uid={creds.get('uid')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_riwayat_ujian(
    ...
    try:
        repo = CbtRepository(db)
        service = CbtService(repo)
        data = await service.get_riwayat_ujian(student_id=student_id)
        ...
        logger.error(f"[cbt/riwayat] Error uid={creds.get('uid')}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
