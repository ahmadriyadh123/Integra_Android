...
class WeeklyPlanService:
    ...
    async def get_weekly_plan_detail(
        ...
        # Fetch Gambar Base64 (Logo & TTD)
        logo_b64 = await self.repo.get_logo_base64()
        ttd_kepsek_b64 = await self.repo.get_attachment_image_base64(
            res_model=f"weekly.plan.{jenjang.lower()}", 
            res_id=plan_id, 
            res_field="ttd_kepsek"
        )
        ttd_guru_b64 = await self.repo.get_attachment_image_base64(
            res_model=f"weekly.plan.{jenjang.lower()}", 
            res_id=plan_id, 
            res_field="ttd_guru"
        )
        ...
        return {
            ...
            "ttd_kepsek_base64": ttd_kepsek_b64,
            "ttd_guru_base64": ttd_guru_b64,
